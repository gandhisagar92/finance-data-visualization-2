...
from src.gql.types.input_types import BuildTreeListInput, ExpandTreeRowInput
...
class TreeQuery(graphene.ObjectType):
    """Tree list queries"""

    # Field definitions
    build_tree_list = graphene.Field(
        TreeListResult, input=graphene.Argument(BuildTreeListInput, required=True)
    )
    expand_tree_row = graphene.Field(
        GraphResult, input=graphene.Argument(ExpandTreeRowInput, required=True)
    )

    def resolve_build_tree_list(self, info, input):
        """
        ...
        meta = TreeListMeta(
            columns=columns, column_groups=None  # TODO: Add support for column groups
        )
    ...
    def resolve_expand_tree_row(self, info, input):
        """
        ...
        # Convert nodes and edges - clean implementation
        nodes = [
        ...
        return GraphResult(nodes=nodes, edges=edges, metadata=None)
