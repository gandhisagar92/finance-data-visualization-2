    ...
    def post(self):
        ...
        try:
            ...
            result = self.graph_service.build_initial_graph(
                ...
                source=source, effective_datetime=effective_datetime
            )
...
class NodeExpandHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            data = self.get_json_body()
            self.validate_required_fields(data, ['nodeId', 'refDataType', 'idType', 'idValue'])
            ...
            result = self.graph_service.expand_node(node_id, ref_data_type, id_type, id_value)
            self.write_json(result)
...
class NodePayloadHandler(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            if not entity:
                self.write_error_response("NODE_NOT_FOUND", f"Node not found: {node_id}", 404)
                return
    ...
    def get(self):
        ...
        try:
            ...
            for provider in self.data_providers.values():
                try:
                    resolved_type = provider.resolve_entity_type(ref_data_type, id_type, id_value)
                    if resolved_type:
                        break
                except Exception:
                    continue
